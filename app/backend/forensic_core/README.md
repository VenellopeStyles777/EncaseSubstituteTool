...
S2-T04/S4.5-IMP03/S4.5-IMP09/S4.5-IMP09A/S4.5-IMP09B do not render previews, export files, recover deleted files, hash evidence, recursively crawl evidence, or require native forensic dependencies for default tests. S4.5-IMP09B provides a live shell-like browser that calls `list_directory()` one path at a time. `FilesystemEntry.allocated` and `FilesystemEntry.deleted` are metadata fields only; they do not expose recoverable byte ranges or content providers.
