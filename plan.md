...
| 4.5 | First testing with user-provided E01 files | Done | S4.5-IMP01 through S4.5-IMP12 are reviewed and done, including demo identity/navigation labels and hash progress/interruption status after 2026-07-24 manual testing. |
| 5 | Add search and timeline foundations | Deferred | Detailed tickets S5-T00 through S5-T16 exist under `tickets/stage-5/`; S5-T00 documentation cleanup is done, S5-T01 rerun is accepted/done with a passed-gate result after an older failed gate, S5-T01A is done, and S5-T02 remains Draft as the next Stage 5 ticket to prepare. |
...
- S4.5-IMP08 can compute SHA-256 over the full EWF logical image stream only when `--hash-image` is explicitly requested; default runs write image hash status `not_run`. S4.5-IMP12 adds progress/status feedback and interrupted non-success artifacts for that long-running path; a completed full-image digest still requires the hash command to finish.
...
- The current backend can write `image-hash.json` and `image-hash-progress.json` with `not_run` by default, can compute a full logical-image SHA-256 when `--hash-image` is explicitly requested, can show progress on stderr, and can record interrupted status without a digest if hashing is stopped before completion.
...
- S4.5-IMP06 reconciles manual-test guardrails and review handoff from S4.5-T07/T08. Status: Done.
- S4.5-IMP07 creates the command-line testing guide with exact PowerShell commands, artifact inspection steps, troubleshooting, and proof boundaries. Status: Done.
...
- S4.5-IMP11 adds project/inspector/custodian identity fields and logical-image navigation labels after 2026-07-24 hands-on demo feedback. Status: Done.
- S4.5-IMP12 adds full-image hash progress/loading-bar and interrupted-status behavior after 2026-07-24 hands-on demo feedback. Status: Done.
...
- S5-T02 is the next Stage 5 ticket to prepare; S5-T03 and later remain blocked until earlier Stage 5 tickets are reviewed.
...
S5-T01 is a hard gate: it must confirm the Stage 4.5 substantial-test implementation runway is complete and reviewed before S5-T02 or later search/timeline implementation proceeds. The 2026-07-16 S5-T01 pass failed this gate because S4.5-IMP01 through S4.5-IMP06 were not complete and reviewed. S4.5-IMP01 through S4.5-IMP12 are now done, and the 2026-07-23 S5-T01 rerun plus later demo-follow-up review leave S5-T02 as the next Stage 5 ticket to prepare.
