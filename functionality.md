...
| First manual E01 testing workflow | 4.5 | Done/Partial | Partial | S4.5-IMP01 through S4.5-IMP07 are reviewed and done. The portable-runtime no-selection workflow has reviewer-run real-image smoke coverage for intake, segment discovery, metadata status, verification status, EWF stream, partition-table volume discovery, root listing, file-list JSON/CSV, static HTML summary, read-only/source-modified assertions, and selected-file `not_run` behavior. Full recursive traversal, broad crawl, and selected-file real-E01 extraction with an approved selection remain untested. |
...
| Search/timeline | 5 | Deferred | Untested | S5-T01 is done with an older failed-gate result; S5-T01A hardened older Stage 4.5 wording; S5-T02 through S5-T16 remain drafted. Search/timeline implementation must wait until S5-T01 is rerun against the completed Stage 4.5 runway. |
...
Manual testing note: Stage 1 through Stage 4 have automated tests but have not been manually exercised by the user as an app workflow. S4.5-IMP01 through S4.5-IMP07 now provide a reviewed command-line first-testing workflow and guide for the no-selection real-image path. Keep full selected-file real-E01 extraction, recursive traversal, and broad crawl untested until those behaviors are exercised with approved selections.
