...
| First manual E01 testing workflow | 4.5 | Done/Partial | Partial | S4.5-IMP01 through S4.5-IMP12 are reviewed and done. S4.5-IMP08 adds explicit `--hash-image` support and `image-hash.json`; S4.5-IMP12 adds `image-hash-progress.json`, stderr progress/status output, and interrupted non-success status without a digest. Default runs keep image hash `not_run`, and a completed real full-image digest is still long-running unless separately completed. S4.5-IMP09/S4.5-IMP09A add bounded nested navigation that can show a regular-file-visible parser-backed directory listing and report nested file paths as `path_not_directory`. S4.5-IMP09B adds a live command-line browser over the same parser-backed listing path. S4.5-IMP11 adds project/inspector/custodian identity plus logical-image navigation labels. S4.5-IMP10 and the updated demo showcase package the hands-on flow. Full recursive traversal, broad crawl, and selected-file real-E01 extraction with an approved selection remain untested. |
...
| Search/timeline | 5 | Deferred | Untested | S5-T01 rerun is accepted/done with a passed-gate result after an older failed-gate result; S5-T01A hardened older Stage 4.5 wording; S4.5-IMP11/S4.5-IMP12 demo follow-ups are reviewed/done; S5-T02 remains drafted as the next Stage 5 ticket to prepare, and S5-T03 through S5-T16 remain drafted. Search/timeline implementation has not started. |
...
Manual testing note: Stage 1 through Stage 4 have automated tests but have not been manually exercised by the user as an app workflow. S4.5-IMP01 through S4.5-IMP12 provide a reviewed command-line first-testing workflow, guide, explicit whole-image hashing command path with progress/interrupted-status artifacts, bounded nested-directory smoke path that can show regular files from the real parser-backed E01 filesystem, live terminal navigation, project/inspector/custodian identity, logical-image navigation labels, and a presentable demo guide. A completed real full-image hash is still a long-running reviewer/user artifact unless separately produced. Keep full selected-file real-E01 extraction, recursive traversal, and broad crawl untested until those behaviors are exercised with approved selections.
