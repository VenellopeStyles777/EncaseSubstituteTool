...
| First manual E01 testing workflow | 4.5 | Active/Partial | Partial | S4.5-IMP01 through S4.5-IMP09A are reviewed and done. S4.5-IMP08 adds explicit `--hash-image` support and `image-hash.json`; default runs keep image hash `not_run`, and the real full-image hash is long-running unless separately completed. S4.5-IMP09/S4.5-IMP09A add bounded nested navigation that can show a regular-file-visible parser-backed directory listing and report nested file paths as `path_not_directory`. Full recursive traversal, broad crawl, and selected-file real-E01 extraction with an approved selection remain untested. |
...
Manual testing note: Stage 1 through Stage 4 have automated tests but have not been manually exercised by the user as an app workflow. S4.5-IMP01 through S4.5-IMP09A provide a reviewed command-line first-testing workflow, guide, explicit whole-image hashing command path, and bounded nested-directory smoke path that can show regular files from the real parser-backed E01 filesystem. A completed real full-image hash is still a long-running reviewer/user artifact unless separately produced. Keep full selected-file real-E01 extraction, recursive traversal, and broad crawl untested until those behaviors are exercised with approved selections.
