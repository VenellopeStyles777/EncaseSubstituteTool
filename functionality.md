...
| First manual E01 testing workflow | 4.5 | Active/Partial | Partial | S4.5-IMP01 through S4.5-IMP07 are reviewed and done. S4.5-IMP08 is ready for review with explicit `--hash-image` support and `image-hash.json`; default runs keep image hash `not_run`, and the real full-image hash is long-running unless separately completed. Required before Stage 5: IMP08 review acceptance, nested directory navigation into actual filesystem entries, and a refreshed guide/gate packet. Full recursive traversal, broad crawl, and selected-file real-E01 extraction with an approved selection remain untested. |
...
| Search/timeline | 5 | Deferred | Untested | S5-T01 is done with an older failed-gate result; S5-T01A hardened older Stage 4.5 wording; S5-T02 through S5-T16 remain drafted. Search/timeline implementation must wait until S4.5-IMP08 through S4.5-IMP10 are reviewed and S5-T01 is rerun. |
...
Manual testing note: Stage 1 through Stage 4 have automated tests but have not been manually exercised by the user as an app workflow. S4.5-IMP01 through S4.5-IMP07 provide a reviewed no-selection command-line first-testing workflow and guide. S4.5-IMP08 adds an explicit independent whole-image hashing command path for review, but a completed real full-image hash is still a long-running reviewer/user artifact unless separately produced. User demo feedback still requires nested directory navigation. Keep full selected-file real-E01 extraction, recursive traversal, and broad crawl untested until those behaviors are exercised with approved selections.
