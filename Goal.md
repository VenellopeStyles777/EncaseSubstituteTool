...
- An added Stage 4.5 first-testing scaffold now lives under `tickets/stage-4.5/`. S4.5-IMP01, S4.5-IMP02, S4.5-IMP02A, S4.5-IMP03, S4.5-IMP04, and S4.5-IMP05 are done as the first implementation slices, including EWF-backed stream, partition-table volume, real-parser-backed root-listing artifacts, explicit selected-file preview/export/hash/signature providers, and root-listing-derived file-list/static local HTML output from the local E01 set. S4.5-IMP06 through S4.5-IMP07 are drafted before any Stage 5 search/timeline work.
